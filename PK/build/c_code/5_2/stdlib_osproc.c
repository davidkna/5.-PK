...
typedef NimStringDesc* TY143060[3];
struct  Tcell46347  {
NI refcount;
TNimType* typ;
};
...
N_NIMCALL(pid_t, startprocessauxfork_143001)(Tstartprocessdata142948* data);
N_CDECL(void, startprocessafterfork_143004)(Tstartprocessdata142948* data);
...
{	result = 0;
	...
	LA4: ;
	pushSafePoint(&TMP45);
	TMP45.status = _setjmp(TMP45.context);
	if (TMP45.status == 0) {
		pid_t volatile pid;
	...
	if (TMP45.status != 0) reraiseException();
	}BeforeRet: ;
	return result;
}
	...
	if (TMP42.status == 0) {
		...
		if (TMP44.status == 0) {
			...
			pid = startprocessauxfork_143001((&data));
			{
				...
				LOC43 = 0;
				LOC43 = nsuJoinSep(args, argsLen0, ((NimStringDesc*) &TMP48));
				printf("%s%s%s\012", command? (command)->data:"nil", ((NimStringDesc*) &TMP48)? (((NimStringDesc*) &TMP48))->data:"nil", LOC43? (LOC43)->data:"nil");
			}
			LA41: ;
			(*result).id = pid;
			{
